...
function formatin(duit){
    ...
    let ratusan = reverse.match(/\d{1,3}/g);
    ratusan = ratusan.join('.').split('').reverse().join('');
    return ratusan;
}
	...
	// Filter Banned People
        ...
        case 'stc' :
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                .then(() => {
                    aruga.reply(from, 'nehh stikernya sayang, bilang apa sama aing?')
                ...
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ...
                    : aruga.reply(from, 'nehh stikernya sayang, bilang apa sama aing?')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
        ...
        case 'sgif' :
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    var mediaData = await decryptMedia(message, uaOverride)
                    aruga.reply(from, 'sabar sayang, lagi diproses ', id)
                    ...
                    await exec(`gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                        ...
                        .catch(() => {
                            aruga.reply(from, 'Maaf,sizenya kegedean. dikecilin lagi yah sayang.', id)
                        })
                    })
                  } else {
        ...
        case 'qtmkr' :
            ...
            if (qmaker.length >= 3) {
                ...
                aruga.reply(from, 'sabar lagi dibuat', id)
                try {
...
${desc}`)
        ...
        case 'nulis':
            ...
            await aruga.sendImage(from, `${nulisp}`, '', 'Nih...iya sama-sama', id)
            .catch(() => {
                aruga.reply(from, 'Ada yang Error!', id)
            })
            break
        ...
        case 'nekopoi':
            ...
            .catch(() => {
                aruga.reply(from, 'Sorry bruhh coli lu tertunda, fitur bacol lu lagi error', id)
            })
            break
        case 'stalkig':
	    ...
	    case 'add':
	        ...
	        if (args.length !== 1) return aruga.reply(from, `Untuk menggunakan ${prefix}add\nPenggunaan: ${prefix}add <nomor>\ncontoh: ${prefix}add 628xxx`, id)
                try {
                    ...
                    aruga.reply(from, 'Target hilang di radar, enemies ahead!', id)
                }
            break
        case 'kick':
            if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
